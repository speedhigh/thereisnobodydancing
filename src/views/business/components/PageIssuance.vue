    ...
    <section class="w-full bg-base-100 rounded-box shadow p-5 h-64">
      ...
      <div class="px-4">
        <div class="mt-[1.88rem] flex items-center space-x-10">
          <!-- 选择产品 -->
          <div class="flex items-center space-x-5 overflow-x-scroll">
            <p class="text-gray-500 flex-shrink-0"><span class="text-primary">*</span>选择产品</p>
            <div
              v-for="(item, index) in proList"
              :key="index"
              class="w-[7.63rem] h-[4.81rem] rounded-btn shadow cursor-pointer flex-shrink-0 relative hover:opacity-80"
            ...
            >
              ...
              <div class="absolute bottom-0 inset-x-0 w-full bg-[rgba(0,0,0,0.5)] text-white text-center text-sm rounded-b-lg leading-6">{{ item.name }}</div>
            </div>
          </div>
        </div>
        <div class="mt-[1.88rem] flex items-center space-x-10">
...
const build = function() {
  ...
  linkData.proid = proList.value[active.value].id
  linkData.href = `http://52.193.133.155:8083/#/order?sid=${user.uuid}&did=${linkData.doctoruuid}&pid=${proList.value[active.value].id}&t=${linkData.biaoshi}`
